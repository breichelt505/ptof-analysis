...
V_exp += (np.random.randn(len(t_exp)) * V_exp.max() * 0.5)
plt.plot(t_exp, V_exp)
sigma = np.ones(len(t_exp)) * V_exp.max() * 0.5


lb = [1e-6,-np.inf,1e-6,1e-6]
ub = [np.inf]*4
p0 = [1.80,1.2,0.5,0.00015]
p_actual = [0.87, 0.0, 0.1, 0.0001]
...
perr_est = np.sqrt(np.diag(pcov))
perr_actual = np.abs(np.array(p_actual)-popt)
pcorr = np.ones_like(pcov)
for i in range(len(popt)):
    for j in range(len(popt)):
        pcorr[i,j] = pcov[i,j]/perr_est[i]/perr_est[j]