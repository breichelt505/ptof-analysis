...
def rchisq(x):
    t_guess, V_guess = output(*x)
    V_interp = interp.interp1d(
        t_guess.flatten(), V_guess.flatten(),bounds_error=False,
        fill_value = 0.0)(t_exp.flatten())
    # print(t_exp.shape, V_exp.shape, V_interp.shape)
    rchisq = np.sum((V_interp-V_exp)**2) / (len(t_exp)-5)
    # plt.plot(t_exp, V_exp,"r--")
    # plt.plot(t_exp, V_interp,"k-")
    return rchisq

sol = opt.minimize(rchisq, [1.0, 0.1, 0.02, 0.004, 0.0002],
