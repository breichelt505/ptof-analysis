...
#%% Important values
DIM = PUT_VAL_HERE #"090-078"
shotnum = PUT_VAL_HERE #"N240104-001-999"
expdir = PUT_VAL_HERE
t_fidu_cal = PUT_VAL_HERE #-382.563
cable_file_path = PUT_VAL_HERE #r".\tables\cable_response\N231017-001-999_deconvolved_cable_chain_IRF.txt"
neut_sensitivity_path = PUT_VAL_HERE #r"tables\detector_response\sensitivities\Neutron Sensitivity.xlsx"
t_delay_cal = PUT_VAL_HERE #210.692
dist = PUT_VAL_HERE #src.units.unitful_to_analysis((417.2)*u.mm)
T_DD = PUT_VAL_HERE #3.03

#%%
src.file_download.download_report_files(shotnum)
src.file_download.download_DIM_config_info(shotnum,DIM, 2)
src.file_download.download_PCD_data(shotnum,DIM, 2)
src.file_download.download_LPOM_files(shotnum)
download_path = PUT_VAL_HERE #r"C:\Users\blr\Downloads"
#%%
exp_ID = src.file_download.get_exp_ID(shotnum,path)
src.file_download.download_nif_server_files(shotnum,exp_ID,download_path,DIM)

#%% Read in files and perform stitching
shifts = np.r_[0.0, 0.145, 0.145+0.162, 0.145+0.162+0.155]
scales = np.r_[2, 4, 8, 16]
file = PUT_VAL_HERE #f"C:\\Users\\blr\\Dropbox (MIT)\\MIT\\Research\\Projects\\ptof_analysis\\scratch\\manual_analysis_tests\\{shotnum}\\TD_TC090-078_PCD_SCOPE-02-DB_{shotnum}.h5"
